...
f1_score <- function(model,data,kernel=TRUE){
  if (kernel){
    ...
    pred <- predict(model,data,proba=TRUE,decisionValues=TRUE)
...
```{r}
nu_list <- seq(0.1,0.9, by = 0.05)
f1_scores <- list()

for(i in 1:length(nu_list)){
  svm.kernel <- svm(cardio ~ ., data=train_model, kernel = "radial",type = "nu-classification",nu=nu_list[i])
  f1_scores <- f1_score(svm.kernel,validation)
}
```
...
### Random Forest
```{r}
target_variable <- "cardio" # Replace with your target variable name
predictor_variables <- setdiff(names(train_model), target_variable)
...
rf_model <- randomForest(
  ...
  mtry = sqrt(length(predictor_variables)) # Number of variables to consider at each split  
)
...
predictions <- predict(rf_model, newdata = validation[-12])
table(predictions)
f1_score(rf_model,validation,TRUE)
